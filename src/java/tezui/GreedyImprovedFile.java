...
import greedy.ImportAircraftGreedy;
 ...
 * @author ulakbim
 */
public class GreedyImprovedFile extends javax.swing.JPanel {
...
//        fcfsTimeLbl.setText("" + fcfsRes[1]);
        ...
        int impRes = greedyImproved(waitingQueue, ints.length);
...
//    static int latestTimes[] = {7, 11, 5, 14, 19, 14, 18};
    ...
    public static int greedyImproved(List<Integer> waitingQueue, int N) {
        ...
        planes = new ImportAircraftGreedy("/Users/ulakbim/Downloads/Aircraftlanding/airland3.txt").getAircraftList();
        int n = planes.size();
        int[][] waitingTimes = new int[n][n];
        for (int i = 0; i < planes.size(); i++) {
            waitingTimes[i] = planes.get(i).getSeperation();
        }
    ...
    public static int isAnyExpiringAircraft(int time, List<AircraftGreedy> planes, int[][] waitingTimes, int candidateType, int lastPlane) {
        ...
        for (int i = 0; i < planes.size(); i++) {
            ...
            if (i != candidateType && !planes.get(i).isLanded() && (planes.get(i).getLatestLandingTime() <= time || planes.get(i).getLatestLandingTime() <= time + timeBeforeCandidate + timeAfterCandidate)) {
                return i;
            }
        }
        return -1;
